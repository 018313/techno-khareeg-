...
(function(){
  ...
  function addHelpButton(){
    const check = setInterval(()=>{
      // دور على منطقة الرئيسية
      const mainAreas = document.querySelectorAll('#app > div, main, [class*="container"]');
      let targetArea = null;
      for(let area of mainAreas){
        if(area.textContent.includes('دفعة تكنولوجيا 2023') || area.textContent.includes('مرحباً')){
          targetArea = area;
          break;
        }
      }
      if(!targetArea) targetArea = document.querySelector('#app > div') || document.body;
      ...
      if(targetArea){
        ...
        helpBtn.innerHTML = '<span style="font-size:22px">❓</span><div style="text-align:right;flex:1"><div style="font-weight:900;color:#f59e0b;font-size:14px">كيف تستخدم المنصة؟</div><div style="font-size:11px;color:#94a3b8;margin-top:2px">اضغط هنا لعرض الجولة التعريفية مرة أخرى</div></div><span style="font-size:18px">▶️</span>';
        helpBtn.onclick = startTour;
        helpBtn.style.cssText = 'display:flex;align-items:center;gap:12px;background:#fffbeb;border:2px dashed #fbbf24;border-radius:18px;padding:16px;width:100%;margin:16px 0;cursor:pointer;font-family:Tajawal,sans-serif;text-align:right;direction:rtl;box-shadow:0 4px 12px rgba(251,191,36,0.15);';
        // حطه فوق في الرئيسية
        if(targetArea.firstChild) targetArea.insertBefore(helpBtn, targetArea.firstChild);
        else targetArea.appendChild(helpBtn);
      }
    }, 800);

    // كمان زرار في صفحة حسابي
    setInterval(()=>{
      const accountPage = document.body.innerHTML.includes('لوحة تحكم الأدمن') || document.body.innerHTML.includes('تغيير الصورة');
      if(accountPage){
        if(document.getElementById('tk-help-btn-account')) return;
        const allButtons = document.querySelectorAll('button');
        let logoutBtn = null;
        for(let b of allButtons){
          if(b.textContent.includes('خروج')){
            logoutBtn = b;
            break;
          }
        }
        if(logoutBtn && logoutBtn.parentElement){
          const helpBtn2 = document.createElement('button');
          helpBtn2.id = 'tk-help-btn-account';
          helpBtn2.innerHTML = '❓ عرض الجولة التعريفية مرة أخرى';
          helpBtn2.onclick = startTour;
          helpBtn2.style.cssText = 'margin-top:10px;background:#f59e0b22;border:1px solid #fbbf24;color:#fbbf24;width:100%;padding:12px;border-radius:99px;font-weight:900;font-size:12px;cursor:pointer;font-family:Tajawal;';
          logoutBtn.parentElement.insertBefore(helpBtn2, logoutBtn);
        }
      }
    }, 1500);
  }
