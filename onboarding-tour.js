...
(function(){
  ...
  function addHelpButton(){
    // شيل أي زرار قديم في حسابي لو موجود
    const oldAcc = document.getElementById('tk-help-btn-account');
    if(oldAcc) oldAcc.remove();
    
    const check = setInterval(()=>{
      // دور على منطقة الرئيسية فقط
      const app = document.getElementById('app');
      if(!app) return;
      
      // تأكد إننا في تبويب الرئيسية (فيه كلمة دفعة تكنولوجيا أو مرحباً أو إحصائيات)
      const isHome = app.textContent.includes('دفعة تكنولوجيا') || app.textContent.includes('مرحباً') || app.textContent.includes('إحصائيات المنصة') || app.textContent.includes('نقاطك');
      if(!isHome) return; // مش في الرئيسية، استنى
      
      if(document.getElementById('tk-help-btn-home')) return;
      
      const helpBtn = document.createElement('button');
      helpBtn.id = 'tk-help-btn-home';
      helpBtn.innerHTML = '<span style="font-size:22px">❓</span><div style="text-align:right;flex:1"><div style="font-weight:900;color:#0f172a;font-size:14px">❓ عرض الجولة التعريفية</div><div style="font-size:11px;color:#475569;margin-top:2px">مش فاهم المنصة؟ دوس هنا وشوف الشرح في دقيقة</div></div><span style="font-size:18px">▶️</span>';
      helpBtn.onclick = function(e){ e.preventDefault(); e.stopPropagation(); startTour(); };
      helpBtn.style.cssText = 'display:flex;align-items:center;gap:12px;background:linear-gradient(135deg,#fffbeb,#fef3c7);border:2px dashed #f59e0b;border-radius:18px;padding:16px;width:calc(100% - 32px);margin:16px auto;cursor:pointer;font-family:Tajawal,sans-serif;text-align:right;direction:rtl;box-shadow:0 4px 12px rgba(245,158,11,0.15);position:relative;z-index:10;';
      
      // حطه في أول الرئيسية بعد الهيدر
      const firstCard = app.querySelector('.glass');
      if(firstCard && firstCard.parentElement){
        firstCard.parentElement.insertBefore(helpBtn, firstCard);
      } else {
        const container = app.querySelector('div.flex-1') || app;
        if(container.firstChild) container.insertBefore(helpBtn, container.firstChild);
        else container.appendChild(helpBtn);
      }
      clearInterval(check);
    }, 500);
    
    // راقب تغيير التبويبات - كل ما يدخل الرئيسية أضف الزرار
    setInterval(()=>{
      const app = document.getElementById('app');
      if(!app) return;
      const isHome = app.textContent.includes('دفعة تكنولوجيا') || app.textContent.includes('مرحباً') || app.textContent.includes('إحصائيات المنصة');
      if(isHome && !document.getElementById('tk-help-btn-home')){
        // حاول تضيفه تاني
        const oldCheck = document.getElementById('tk-help-btn-home');
        if(!oldCheck){
          const helpBtn = document.createElement('button');
          helpBtn.id = 'tk-help-btn-home';
          helpBtn.innerHTML = '<span style="font-size:22px">❓</span><div style="text-align:right;flex:1"><div style="font-weight:900;color:#0f172a;font-size:14px">❓ عرض الجولة التعريفية</div><div style="font-size:11px;color:#475569;margin-top:2px">مش فاهم المنصة؟ دوس هنا وشوف الشرح في دقيقة</div></div><span style="font-size:18px">▶️</span>';
      ...
      // لو في حسابي، شيل أي زرار قديم
      const accBtn = document.getElementById('tk-help-btn-account');
