// تكنو خريج - نظام الجولة التعريفية - نسخة SpotLight واضحة v13
(function(){
  const STEPS = [
  ...
  function highlightTarget(target){
    ...
    if(target.closest && target.closest('nav')){
      ...
      // أظهر لابل عائم فوق الأيقونة باسمها
      try{
        let rect = target.getBoundingClientRect();
        let lbl = document.getElementById('tk-nav-label');
        if(lbl){
          let name = target.textContent.trim().replace(/\d+/g,'').trim();
          // استخرج اسم التاب من onclick لو أمكن
          let oc = target.getAttribute('onclick')||'';
          let m = oc.match(/switchTab\('([^']+)'\)/);
          if(m) name = m[1];
          lbl.textContent = name;
          lbl.style.left = (rect.left + rect.width/2) + 'px';
          lbl.style.top = (rect.top - 38) + 'px';
